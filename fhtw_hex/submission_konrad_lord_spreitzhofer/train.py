...
def train_model(board_size=config.BOARD_SIZE, epochs=config.EPOCHS, num_games_per_epoch=config.NUM_OF_GAMES_PER_EPOCH):
    ...
    for epoch in range(1, epochs + 1):
        ...
        win_rates_checkpoint, avg_moves_checkpoint = [], []  # Initialize variables
        ...
        print(f"Completed Epoch {epoch}/{epochs} with Policy Loss: {policy_loss.item()}, Value Loss: {value_loss.item()}, Win Rates: {win_rates_checkpoint}, Avg. Moves: {avg_moves_checkpoint}")
